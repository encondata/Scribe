...
# Google Sheets CSV export URL (replace with your actual URL)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Source Location
        col = QVBoxLayout(); col.setSpacing(0)
        lbl = QLabel("Scanning Location"); lbl.setAlignment(Qt.AlignCenter)
    ...
    def download_ft(self):
        ...
        if os.path.isfile('current_ft.csv'):
            ts = datetime.now().strftime('%m%d_%H%M')
            os.rename('current_ft.csv', f'ft_{ts}.csv')

        try:
            resp = requests.get(SHEETS_CSV_URL)
            resp.raise_for_status()
            with open('current_ft.csv', 'wb') as f:
