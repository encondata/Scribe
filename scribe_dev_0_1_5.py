...
SHEETS_CSV_URL = 'https://docs.google.com/spreadsheets/d/1nZRogjTXEbYqKk-Tq7aNbWni2s7ukpqKHAPaIkkL2NE/export?format=csv&gid=0'

# Dropdown options
label_type_options      = ['Top', 'Rail', 'Crate', 'Front', 'Front - 203dpi', 'Priority', 'All']
label_format_options    = ['Vegas', 'Amsterdam', 'Bengaluru', 'Ashburn', 'Tokyo', 'Row / Rack', 'Custom']
num_to_print_options    = ['1', '2', '3', '4']
move_name_options       = ['Future Use', 'Future Use']
source_location_options = ['Pre-Staging','Scanning Station', 'Loading Dock', '----------', 'Unpacking', 'Sorting', 'QA', 'Final']
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        # set window icon
        self.setWindowIcon(QIcon('scribe_logo.png'))
        ...
        # Move Name
        col = QVBoxLayout(); col.setSpacing(0)
        lbl = QLabel("Move Name"); lbl.setAlignment(Qt.AlignCenter)
        combo = QComboBox(); combo.addItems(move_name_options); combo.setFixedWidth(COMBO_WIDTH)
        self.move_name = combo
        col.addWidget(lbl); col.addWidget(combo)
        row2.addLayout(col)
        # Source Location
        col = QVBoxLayout(); col.setSpacing(0)
        lbl = QLabel("Source Location"); lbl.setAlignment(Qt.AlignCenter)
        combo = QComboBox(); combo.addItems(source_location_options); combo.setFixedWidth(COMBO_WIDTH)
        self.source_location = combo
        col.addWidget(lbl); col.addWidget(combo)
        row2.addLayout(col)
        # Printer Selection
        col = QVBoxLayout(); col.setSpacing(0)
        lbl = QLabel("Printer Selection"); lbl.setAlignment(Qt.AlignCenter)
        combo = QComboBox(); combo.setFixedWidth(COMBO_WIDTH)
        combo.addItem(f"USB - {self.printer_usb_name}", "USB")
        combo.addItem(f"Bluetooth - Future{self.printer_bluetooth_mac}", "Bluetooth")
        combo.addItem(f"IP - {self.printer_ip_address}", "IP")
        self.printer_selection = combo
        col.addWidget(lbl); col.addWidget(combo)
        row2.addLayout(col)
        ml.addLayout(row2)
        ml.addSpacing(20)
        ...
        # clickable "Labels Printed"
        self.lbl_labels = ClickableLabel(f"Labels Printed: {self.labels_printed}")
        self.lbl_labels.setAlignment(Qt.AlignCenter)
        self.lbl_labels.clicked.connect(self.reset_labels_counter)
        sl.addWidget(self.lbl_labels)
    ...
    def reset_labels_counter(self):
        """Reset printed-count to zero when label clicked."""
        self.labels_printed = 0
        self.update_status_bar()
        ...
        if os.path.isfile(SETTINGS_FILE):
            with open(SETTINGS_FILE) as f:
                for line in f:
                    ...
                    elif k == 'MQTT_TOPIC':      self.mqtt_topic = v
    ...
    def status_checks(self):
        ...
        try:
            s = socket.create_connection((self.mqtt_server, self.mqtt_port), timeout=5); s.close(); self.status_mqtt = True
        except:
            self.status_mqtt = False
        try:
            s = socket.create_connection((self.sql_server, self.sql_port), timeout=5); s.close(); self.status_sql = True
        except:
            self.status_sql = False
        self.update_status_bar()
    ...
    def download_ft(self):
        if not self.status_internet:
            self.show_error_popup("Failed to Download.\nVerify Internet Connection.")
            return
        if os.path.isfile(CURRENT_FT_FILE):
            ts = datetime.now().strftime('%m%d_%H%M')
            os.rename(CURRENT_FT_FILE, f'ft_{ts}.csv')
        try:
            resp = requests.get(SHEETS_CSV_URL)
            resp.raise_for_status()
            with open(CURRENT_FT_FILE, 'wb') as f:
                f.write(resp.content)
            self.show_success_popup("Download Successful")
        except Exception:
            self.show_error_popup("Failed to Download.\nVerify Internet Connection.")
    ...
    def show_error_popup(self, message):
        ...
        font = QFont(); font.setPointSize(24); font.setBold(True); lbl.setFont(font)
        lbl.setAlignment(Qt.AlignCenter)
        layout = QVBoxLayout(dlg); layout.addWidget(lbl)
        dlg.setFixedSize(640, 480); dlg.show()
    ...
    def record_scan(self):
        ...
        # handle "all" to print every row
        if text.lower() == 'print all labels':
            if os.path.isfile(CURRENT_FT_FILE):
                with open(CURRENT_FT_FILE, newline='') as f:
                    dr = csv.DictReader(f)
                    for row in dr:
                        self.generate_zpl_and_print(row)
                self.show_success_popup("Printing All Labels")
            else:
                self.show_error_popup("No data file found.")
        ...
        row_log = [
            ...
            self.printer_selection.currentData(),
        ...
        with open(SCAN_LOG_FILE, 'a', newline='') as f:
            ...
            if newf:
                w.writerow([
                    'scan_code','label_type','label_format','num_to_print',
                    'move_name','source_location','printer_selection','timestamp'
                ])
            w.writerow(row_log)
        ...
        if not found:
            QApplication.beep()
            self.show_error_popup("No Match Found")
            ltype = "Not Found"
        else:
            QApplication.beep()
            self.generate_zpl_and_print(lookup_row)
            self.show_success_popup("Match Found")
        ...
        if self.status_internet and self.status_mqtt and self.mqtt_topic:
            self.send_mqtt_message(text, self.source_location.currentText())

        self.load_lookup_log()
        self.scan_entry.clear()
        self.scan_entry.setFocus()

    def send_mqtt_message(self, scan_value, location_value):
        payload = {
            "scan": scan_value,
        ...
        try:
            ...
            client.publish(self.mqtt_topic, json.dumps(payload))
            client.disconnect()
    ...
    def generate_zpl_and_print(self, row):
        ...
        if '^XZ' in zpl_data:
            ...
            zpl_data += f'\n^PQ{count}^XZ'
        sel = self.printer_selection.currentData()
        try:
            if sel == 'USB':
                z = Zebra(self.printer_usb_name)
                z.output(zpl_data)
            elif sel == 'Bluetooth':
                z = Zebra(self.printer_bluetooth_mac)
                z.output(zpl_data)
            else:
...
if __name__ == '__main__':
    ...
    app.setWindowIcon(QIcon('scribe_logo.png'))
    win = MainWindow()
    win.show()
    sys.exit(app.exec())
